...
const GITIGNORE_CONTENT: &str = "# Private area — not tracked by git\n.private/\n";

/// Default `README.md` content for `_knowledge/`.
const README_CONTENT: &str = "# Knowledge Base\n\nThis directory contains project knowledge documents, templates, and skills.\n\n## Directory Structure\n\n- `docs/` — Technical documentation\n- `templates/` — Document and prompt templates\n- `skills/` — AI skill definitions (each skill is a subdirectory with `skill.yaml`)\n- `requirements/` — Requirement source materials\n- `.private/` — Local private area (not tracked by git)\n  - `ai-intermediate/` — AI-generated intermediate drafts\n  - `personal-notes/` — Personal notes\n\n## Skills Format\n\nEach skill in `skills/` is a directory containing:\n- `skill.yaml` — Skill metadata and trigger rules\n- `template.md` — Prompt template\n\nSee the project documentation for more details.\n";
...
mod tests {
    ...
    fn test_init_kb_dir_new() {
        ...
        assert!(readme.contains("Knowledge Base"));
    }
