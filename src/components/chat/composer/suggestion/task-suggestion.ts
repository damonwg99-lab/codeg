...
export interface TaskSuggestionRenderState {
  query: string
 ...
 */
export interface TaskSuggestionController {
  onStart: (state: TaskSuggestionRenderState) => void
  onUpdate: (state: TaskSuggestionRenderState) => void
  onExit: () => void
...
const NOOP_CONTROLLER: TaskSuggestionController = {
  onStart: () => {},
  onUpdate: () => {},
  onExit: () => {},
  onKeyDown: () => false,
...
function toRenderState(props: SuggestionProps): TaskSuggestionRenderState {
  return {
    query: props.query,
    range: props.range,
 ...
 * {@link TaskSuggestionController}. Data fetching, rendering and insertion are
 * handled by the controller's React popup, so the plugin's own `items`/`command`
 * are intentionally inert.
 */
export const TaskSuggestion = Extension.create<TaskSuggestionOptions>({
  name: "taskSuggestion",
