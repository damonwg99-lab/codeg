...
import { useContentFileSearch } from "@/hooks/use-content-file-search"
import { filterFileEntriesForSearch } from "@/lib/file-search-filter"
...
export function SearchCommandDialog({
  ...
  // Content search (files tab) — fork feature, fully self-contained hook:
  // desktop streams batches over the `search_files_content:results` event,
  // web issues a single HTTP call; debouncing, aborting and reset-on-close
  // live inside the hook.
  const { contentResults, contentSearching } = useContentFileSearch({
    open,
  ...
  // Filter files by query — the strategy (browse mode on empty query,
  // substring match otherwise) lives in `@/lib/file-search-filter` so main's
  // component refactors never touch it.
  const filteredFiles = useMemo(
    () => filterFileEntriesForSearch(query, allFiles),
    [allFiles, query]
  )
        ...
        {activeTab === "files" && (
          <>
            ...
            <ContentSearchResults
              headingLabel={t("contentMatches")}
              searchingLabel={t("searching")}
              searching={contentSearching}
              results={contentResults}
              onSelect={handleSelectContentMatch}
            />
          </>
        )}
      </CommandList>
