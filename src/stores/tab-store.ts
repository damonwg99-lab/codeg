...
// PLATFORM CUSTOM BEGIN: PendingTaskLink + pending* state lives in the
// separate `platform-tab-slice` store to avoid conflicting with upstream
// tab-store evolution. Re-exported here for backwards-compat with consumers
// that import the type from @/stores/tab-store.
export type { PendingTaskLink } from "@/stores/platform-tab-slice"
// PLATFORM CUSTOM END.
...
export interface TabStoreState {
  ...
  // PLATFORM CUSTOM BEGIN: pendingInitialDrafts / pendingTaskLink moved to
  // the separate `platform-tab-slice` store — see stores/platform-tab-slice.ts.
  // PLATFORM CUSTOM END.
  /**
  ...
  // PLATFORM CUSTOM BEGIN: setPendingInitialDraft / clearPendingInitialDraft /
  // setPendingTaskLink / clearPendingTaskLink moved to the separate
  // `platform-tab-slice` store — see stores/platform-tab-slice.ts.
  // PLATFORM CUSTOM END.
...
function makeReplacementDraftTab(preferred?: TabItemInternal): TabItemInternal {
  ...
  const preferredIsChat =
    ...
    // PLATFORM CUSTOM BEGIN: platform_repo folders are also hidden —
    // see `isHiddenFolderKind` in lib/platform/types.ts.
    isHiddenFolderKind(
      allFolders.find((f) => f.id === preferred?.folderId)?.kind
    )
  // PLATFORM CUSTOM END.
  const nonChatFallbackId =
    // PLATFORM CUSTOM BEGIN: platform_repo excluded from fallback pool.
    folders.find((f) => !isHiddenFolderKind(f.kind))?.id ?? 0
  // PLATFORM CUSTOM END.
...
function initialTabState() {
  return {
    ...
    // PLATFORM CUSTOM BEGIN: pendingInitialDrafts / pendingTaskLink init lives
    // in the separate `platform-tab-slice` store now.
    // PLATFORM CUSTOM END.
...
export const useTabStore = create<TabStoreState>()((set, get) => ({
  ...
  // PLATFORM CUSTOM BEGIN: setPendingInitialDraft / clearPendingInitialDraft /
  // setPendingTaskLink / clearPendingTaskLink implementations moved to the
  // separate `platform-tab-slice` store — see stores/platform-tab-slice.ts.
  // PLATFORM CUSTOM END.
...
export function useTabActions() {
  return useTabStore(
    useShallow((s) => ({
      ...
      // PLATFORM CUSTOM BEGIN: pending* actions exposed via the merged
      // `useTabActions` in tab-context.tsx (pulls from platform-tab-slice).
      // PLATFORM CUSTOM END.
    }))
  )
}
