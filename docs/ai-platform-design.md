...
pub struct Model {
    ...
    pub kb_local_dir: Option<String>,    // 知识库本地目录（默认: root_dir/_knowledge/）
...
pub struct Model {
    ...
    pub injected_docs_json: Option<String>, // 注入的文档列表(JSON)，便于追溯
                                           // 如: ["_knowledge/docs/architecture/product-arch.md", "/path/to/CLAUDE.md"]
    pub created_at: DateTimeUtc,
    pub updated_at: DateTimeUtc,
}
...
│  1. 从GitLab clone到项目根目录/_knowledge/             │
...
**git扫描排除规则**：扫描项目根目录时，跳过 `_knowledge/` 目录（知识库仓库）
...
│  ┌──────────────────────────────────────┐      │
│  │ 状态: 已初始化 (本地)                    │      │
│  │ 路径: /workspace/tpm-client-A/_knowledge/     │      │
